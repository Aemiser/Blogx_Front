...
7. **新增**：文章导航组件实现建议：
   ...
   - 功能要求：
     ...
     * 显示层级缩进的目录项，支持折叠/展开
     * 点击目录项时平滑滚动到对应标题位置，**确保标题恰好显示在导航栏下方**
   ...
   - **关键实现**：目录点击定位时确保标题显示在导航栏下方
     ```typescript
     // src/components/article/nav/ArticleNav.vue - 脚本部分
     function scrollToHeader(header) {
       // 更新激活状态（立即反馈）
       headers.value.forEach(h => h.isActive = false)
       ...
       // 获取页面顶部固定元素的总高度（如头部导航）
       const fixedSelectors = ['.fixed-header', '.site-header', 'header', '.top-nav']
       ...
       // 总偏移量 = 导航栏高度 + 顶部固定元素高度 + 额外间距
       // 这样确保目标标题会显示在导航栏正下方
       const totalOffset = navBarHeight + fixedHeaderHeight + 16 // 16px额外间距
...
6. **新增**：文章导航功能测试重点
   - 单元测试：标题提取工具函数的正确性
   - 组件测试：ArticleNav的目录生成和点击定位功能（**重点测试定位准确性**）
   - 组件测试：滚动监听和激活状态更新
   - E2E测试：文章导航完整交互流程（点击目录项定位、滚动激活等）
   - **特别测试点**：验证点击目录项后，目标标题恰好显示在导航栏下方
...
- **文章导航特别注意**：
  ...
  8. **定位精度优化**：目录点击定位后，**必须确保目标标题恰好显示在导航栏下方**，不被导航栏遮挡，也不留有过大的空白区域
     * 动态计算导航栏高度而非使用固定值
     * 考虑页面顶部其他固定元素的高度
     * 添加适当的间距以获得最佳视觉效果
     * 在不同视口大小和缩放级别下测试定位准确性